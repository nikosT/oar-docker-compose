...
# activate only if -t exclusive is assigned in oarsub
#A chieving exclusivness at the granularity of node

if 'exclusive' in types and 'coschedule' in types:
    ...
    print("#ADMISSION RULE>Allocating entire node")
    ...
    # round upwards
    cpus=math.ceil(cores/(cores_per_cpu))
    # even number of cores per cpu is acceptable
    nodes=math.ceil((cpus*2)/(cpus_per_node))
    
    # convert outcome to OAR guidelines
    resources=[{'resource': 'network_address', 'value': nodes}]
